...
struct Proposal {
    ...
    uint256 newValue;            // value to apply
    address collectionAddress;   // context for TIER_UPGRADE or voting-weight attribution
    address proposer;
    uint256 startBlock;
    uint256 endBlock;
    uint256 votesScaled;         // accumulated votes (scaled by WEIGHT_SCALE)
    bool executed;
}

struct CollectionConfig {
    uint256 totalStaked;     // number of NFTs staked
    uint256 totalStakers;    // number of distinct stakers
    bool registered;
    uint256 declaredSupply;  // collection max supply declared during registration
}
...
struct CollectionMeta {
    CollectionTier tier;
    address registrant;
    uint256 surchargeEscrow;        // held in contract for UNVERIFIED collections
    uint256 registeredAtBlock;
    uint256 lastTierProposalBlock;  // cooldown for tier proposals
}
...
// Parameter update events (useful for off-chain indexing)
