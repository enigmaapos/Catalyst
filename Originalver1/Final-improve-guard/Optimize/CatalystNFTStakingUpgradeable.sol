    ...
    IERC721Receiver
{
    ...
    uint256 public constant MAX_BATCH = 200; // used for stake/unstake/harvest batch caps
    ...
    // -------- Protocol params (internal to reduce autogenerated getter bytecode) --------
    // NOTE: visibility changed to internal to avoid many auto-generated getters.
    uint256 internal numberOfBlocksPerRewardUnit;
    uint256 internal termDurationBlocks;
    uint256 internal stakingCooldownBlocks;
    uint256 internal rewardRateIncrementPerNFT;
    uint256 internal initialHarvestBurnFeeRate;
    uint256 internal unstakeBurnFee;
    uint256 internal collectionRegistrationFee;
    address internal treasuryAddress;
    address internal deployerAddress;
    uint256 internal treasuryBalance;
    uint256 internal minStakeAgeForVoting;
    uint256 internal maxBaseRewardRate;
    address[] internal registeredCollections;
    mapping(address => uint256) internal registeredIndex;
...
// -------- Registration (permissionless with fee guard) --------
    ...
    function _checkNonZeroAndRegistered(address collection) internal view {
        if (collection == address(0)) revert ZeroAddress();
        if (!s.collectionConfigs[collection].registered) revert NotRegistered();
    }
    ...
    function stake(address collection, uint256 tokenId, bool permanent)
        ...
        notInCooldown
    {
        _checkNonZeroAndRegistered(collection);
    ...
    // Consolidated reward processing to avoid duplicated logic
    function _processReward(address owner, uint256 reward, address collection) internal {
        if (reward == 0) return;
        ...
        if (mintAmount > 0) _mint(owner, mintAmount);
        if (burnAmt > 0) {
            _burn(owner, burnAmt);
            burnedCatalystByAddress[owner] += burnAmt;
        }
        emit RewardsHarvested(owner, collection, mintAmount, burnAmt);
    }
    ...
    function harvest(address collection, uint256 tokenId) public whenNotPaused nonReentrant {
        ...
        _processReward(_msgSender(), reward, collection);
    ...
    function harvestBatch(address collection, uint256[] calldata tokenIds) external whenNotPaused nonReentrant {
        uint256 n = tokenIds.length;
        if (n == 0 || n > MAX_BATCH) revert BadParam(); // safety cap for front-end
        for (uint256 i = 0; i < n; ++i) {
    ...
    function unstakeBatch(address collection, uint256[] calldata tokenIds) external whenNotPaused nonReentrant {
        uint256 n = tokenIds.length;
        if (n == 0 || n > MAX_BATCH) revert BadParam();
        for (uint256 i = 0; i < n; ++i) {
    ...
    function harvestBluechip(address collection) external whenNotPaused nonReentrant {
        ...
        // delegate to BluechipLib which will call back into this contract to mint via _mintReward
    ...
    /// @notice Returns aggregated stats for the protocol and a specific collection.
    function getProtocolStats(address collection, address wallet)
        external
        view
        returns (
            uint256 totalAll,
            ...
            uint256 collectionCount_,
            uint256 collectionTotal_,
            bool isBluechip,
            bool isEnrolled,
            uint8 collectionTier
        )
    {
        ...
        collectionCount_ = registeredCollections.length;
        collectionTotal_ = s.collectionTotalStaked[collection];
        isBluechip = b.isBluechipCollection[collection];
        isEnrolled = b.bluechipWallets[address(0)][wallet].enrolled;
    ...
    /// @notice Returns all information about a specific governance proposal.
