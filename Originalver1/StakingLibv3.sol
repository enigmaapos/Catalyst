...
library StakingLib {
    ...
    function recordPermanentStake(
        ...
        info.unstakeDeadlineBlock = type(uint256).max;
    ...
    function recordUnstake(Storage storage s, address collection, address staker, uint256 tokenId, bool wasPermanent) external {
        ...
        uint256 lastIndex = s.stakePortfolioByUser[collection][staker].length - 1;
        uint256 tokenIndex = s.indexOfTokenIdInStakePortfolio[collection][tokenId];
        uint256 lastTokenId = s.stakePortfolioByUser[collection][staker][lastIndex];
        s.stakePortfolioByUser[collection][staker][tokenIndex] = lastTokenId;
        s.indexOfTokenIdInStakePortfolio[collection][lastTokenId] = tokenIndex;
        s.stakePortfolioByUser[collection][staker].pop();
