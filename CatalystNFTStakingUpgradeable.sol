...
/// @title Catalyst NFT Staking Protocol (Upgradeable, Lean)
/// @notice Minimal upgradeable skeleton with staking + governance.
/// @dev Use Timelock + governance to secure upgrades and treasury withdrawals.
    ...
    UUPSUpgradeable
{
    ...
    /// @notice Initialize contract (proxy)
    function initialize(
        address admin_,
        address deployer_,
        ...
        GovernanceLib.initGov(g, votingDurationBlocks, minVotesRequiredScaled, collectionVoteCapPercent);
        ConfigLib.setUint(c, 12, 18782); // default reward timing
    }
    ...
    function termStake(address collection, uint256 tokenId) external nonReentrant whenNotPaused {
        ...
        require(s.collectionConfigs[collection].registered, "not registered");
    ...
    function permanentStake(address collection, uint256 tokenId) external nonReentrant whenNotPaused {
        ...
        require(balanceOf(_msgSender()) >= fee, "insufficient");
        ...
        if (treasuryAmt > 0) {
            _transfer(_msgSender(), address(this), treasuryAmt);
            TreasuryLib.recordDeposit(t, _msgSender(), treasuryAmt);
        }

        StakingLib.recordPermanentStake(s, collection, _msgSender(), tokenId, block.number, ConfigLib.getUint(c, 21));
    ...
    function unstake(address collection, uint256 tokenId) external nonReentrant whenNotPaused {
        ...
        uint256 burned = 0;
        if (reward > 0) {
            uint256 feeRate = _getHarvestBurnFeeRate();
    ...
    function propose(
        ...
        bytes32 id = GovernanceLib.createProposal(
            g, pType, paramTarget, newValue, collectionContext, _msgSender(), block.number
        );
        emit ProposalCreated(id, pType, paramTarget, newValue);
        return id;
    }

    function vote(bytes32 id, uint256 weight) external whenNotPaused {
        GovernanceLib.castVote(g, id, _msgSender(), weight, address(0));
    }

    function executeProposal(bytes32 id) external whenNotPaused {
    ...
    // === Fee Handling ===
    function handleFee(address from, uint256 amount) external nonReentrant {
    ...
    // === Internal Helpers ===
    function _getHarvestBurnFeeRate() internal view returns (uint256) {
        return ConfigLib.getUint(c, 14); // base harvest burn fee
    }
    ...
    // === UUPS Auth (Timelock should be DEFAULT_ADMIN_ROLE) ===
