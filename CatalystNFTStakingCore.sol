    ...
    IERC721Receiver
{
    ...
    function stake(address collection, uint256 tokenId, bool permanent) public nonReentrant whenNotPaused {
        ...
        if (permanent) {
            ...
            uint256 fee = ConfigRegistryLib.getDynamicPermanentStakeFee(config, staking.totalStakedNFTsCount);
    ...
    // batch wrappers (fixed: inline per-token logic; no _stake helper)
    ...
    // harvest single
    function harvest(address collection, uint256 tokenId) public nonReentrant whenNotPaused {
    ...
    // harvest batch (fixed: inline per-token harvest)
    ...
    function registerCollection(address collection, uint256 declaredMaxSupply, ConfigRegistryLib.CollectionTier requestedTier) external nonReentrant whenNotPaused {
        (uint256 baseFee, uint256 surcharge) = ConfigRegistryLib._computeRegistrationFees(config, declaredMaxSupply, requestedTier);
    ...
    function setCollectionConfig(address collection, uint256 declaredMaxSupply, ConfigRegistryLib.CollectionTier tier) external onlyRole(CONTRACT_ADMIN_ROLE) nonReentrant whenNotPaused {
        (uint256 baseFee, uint256 surcharge) = ConfigRegistryLib._computeRegistrationFees(config, declaredMaxSupply, tier);
